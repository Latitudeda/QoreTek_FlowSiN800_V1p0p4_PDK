...
#. ``CircularBendFactory`` : Import ``BendCircular`` from ``bend_circular`` and assigned each component to different situations.
    ...
    * Note that the default bend radius for every 90-degree bend used for auto-routing is set to a recommended value of 30 um. However, users should define the bend radius themselves to meet the design requirements.
